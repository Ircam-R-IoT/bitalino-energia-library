...
boolean BITalinoClass::start(int samplingRate = 1000, byte channelsBitmap = 0x3F, bool simulated = false)
{
    ...
    Serial1.write((uint8_t)((cmd << 6) | 0x03));

    delay(50);

    // send "start" command to device
    Serial1.write((uint8_t)(chBmp << 2) | (simulated ? 0x02 : 0x01));
...
word BITalinoClass::read(word nFrames, BITalinoFrame frames[])
{
    ...
    {
      if (Serial1.readBytes((char *) buffer, nBytes) < nBytes)  return fCnt;  // return previous frames if timeout occurred
...
word BITalinoClass::readSingle(BITalinoFrame *frame)
{
	...
	if(Serial1.readBytes(SerialBuffer, nBytes) < nBytes)
        return 0;
    else
    {
        SerialIndex += nBytes;
        ...
        if(!BITalino.checkCRC4(SerialBuffer, nBytes))
        {
          ...
          {   // if CRC check failed, try to resynchronize with the next valid frame
             ...
             {
                Serial1.readBytes(SerialBuffer, 1);  // reads one byte with timeout
             }
             Attempt--;
          }
        }
...
boolean BITalinoClass::trigger(byte outputsBitmap = 0)
{
    ...
    Serial1.write((uint8_t)(((outputsBitmap & 0x0F) << 2) | 0x03));
